...
class NotionAssignmentMatcher:
    ...
    def extract_activities(self, notion_results: Any) -> List[Dict]:
        ...
        for item in items:
            ...
            if title and activity_id:
                activities.append({"title": title, "id": activity_id, "teacher": ""})
        return activities
    ...
    def run(self):
        ...
        if matched_activity_id:
            print(
                f"Assignment '{sample_assignment['assignment_name']}' matches activity with ID: {matched_activity_id}"
            )
        else:
            print(
                f"No matching activity found for assignment '{sample_assignment['assignment_name']}'"
            )
